...
// DDDGenerator DDD 架构代码生成器
type DDDGenerator struct {
	config Config
}

// NewDDDGenerator 创建 DDD 生成器
...
// generateDomainLayer 生成 Domain 层
...
// {{.ModelName}} {{.ModelName}} 领域实体
//
// 职责说明：
//   - 封装 {{.ModelName}} 的业务规则和行为
//   - 确保领域对象的完整性和一致性
//   - 不包含任何基础设施细节（如数据库、缓存）
//
// 设计原则：
...
// New{{.ModelName}} 创建 {{.ModelName}} 聚合根
//
// 参数：
//   username - 用户名
//   email - 邮箱
//
// 返回：
//   *{{.ModelName}} - 新创建的 {{.ModelName}} 实体
//   error - 参数验证失败时返回错误
func New{{.ModelName}}(/* TODO: 添加参数 */) (*{{.ModelName}}, error) {
	entity := &{{.ModelName}}{
		// TODO: 初始化字段
	}
...
type {{.ModelName}}Repository interface {
	...
	// 参数：
	//   ctx - 上下文
	//   {{ToLowerCamelCase .ModelName}} - {{.ModelName}} 聚合根
	//
	// 返回：
	//   error - 保存失败时返回错误
	Save(ctx context.Context, {{ToLowerCamelCase .ModelName}} *{{.ModelName}}) error

	// FindByID 根据 ID 查找 {{.ModelName}}
	//
	// 返回：
	//   *{{.ModelName}} - {{.ModelName}} 聚合根，不存在时返回 nil
	//   error - 查询失败时返回错误
	FindByID(ctx context.Context, id uint) (*{{.ModelName}}, error)

	// FindAll 查找所有 {{.ModelName}}
	//
	// 返回：
	//   []*{{.ModelName}} - {{.ModelName}} 列表
	//   error - 查询失败时返回错误
...
// {{.ModelName}}Service {{.ModelName}} 领域服务
//
// 职责说明：
//   - 实现不属于某个实体的业务逻辑
//   - 协调多个实体或值对象完成业务操作
...
// New{{.ModelName}}Service 创建 {{.ModelName}} 领域服务
func New{{.ModelName}}Service(/* TODO: 添加参数 */) *{{.ModelName}}Service {
...
// generateApplicationLayer 生成 Application 层
...
// generateApplicationService 生成应用服务
...
// {{.ModelName}}Service {{.ModelName}} 应用服务
//
// 职责说明：
//   - 实现 {{.ModelName}} 的用例（Use Case）
//   - 协调领域对象完成业务操作
//   - 处理事务边界
//   - 与基础设施交互（通过仓储接口）
//
...
type {{.ModelName}}Service struct {
	...
	// TODO: 添加其他依赖（如其他聚合的仓储）
}

// New{{.ModelName}}Service 创建 {{.ModelName}} 应用服务
...
// 例如：
//
// // Create{{.ModelName}} 创建 {{.ModelName}} 用例
...
// }
//
// // Update{{.ModelName}} 更新 {{.ModelName}} 用例
...
// Create{{.ModelName}}Command 创建 {{.ModelName}} 命令
type Create{{.ModelName}}Command struct {
	// TODO: 添加字段
}

// Update{{.ModelName}}Command 更新 {{.ModelName}} 命令
...
// generateInfrastructureLayer 生成 Infrastructure 层
...
// New{{.ModelName}}RepositoryImpl 创建 {{.ModelName}} 仓储实现
...
// generateInterfaceLayer 生成 Interface 层
...
// generateHTTPController 生成 HTTP 控制器
...
// {{.ModelName}}Controller {{.ModelName}} HTTP 控制器
//
// 职责说明：
...
// RegisterDDDRoutes 注册 DDD 架构的路由
//
