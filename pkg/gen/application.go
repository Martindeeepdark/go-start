...
`
	...
	if err != nil {
		return fmt.Errorf("解析 application 模板失败: %w", err)
	}

	f, err := os.Create(outputPath)
	if err != nil {
		return fmt.Errorf("创建 application.go 失败: %w", err)
	}
	defer f.Close()
	...
	if err := t.Execute(f, data); err != nil {
		return fmt.Errorf("执行 application 模板失败: %w", err)
	}

	fmt.Println("     ✓ internal/application/application.go 创建成功")
	return nil
}
